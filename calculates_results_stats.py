...
def calculates_results_stats(results_dic):
    ...
    for key in results_dic:
        print(f"Key: {key}, Value: {results_dic[key]}")
        if len(results_dic[key]) < 5:
            print(f"Error: Entry for key '{key}' is malformed. Expected a list of length 5, got {len(results_dic[key])}.")
        ...
        results_stats_dic['pct_match'] = 100.0 * (results_stats_dic['n_match'] /
                                                 results_stats_dic['n_images'])

        if results_stats_dic['n_dogs_img'] > 0:
        # Calculates % correct dog
            results_stats_dic['pct_correct_dogs'] = 100.0 * (results_stats_dic['n_correct_dogs'] /
                                                 results_stats_dic['n_dogs_img'])
